...
def test_run_sync_writes_key_takeaway_from_chart_tab(tmp_path):
    # The students-all fixture carries KEY TAKEAWAY rows, so the written JSON
    # should surface them end-to-end (chart tab → parser → writer).
    client = make_fake_client()
    result = run_sync(client, out_dir=tmp_path, dry_run=False)
    assert result["status"] == "success"
    written = json.loads((tmp_path / "students-all.json").read_text(encoding="utf-8"))
    assert written["key_takeaway"] == {"th": "ประเด็นสำคัญทดสอบ", "en": "Test takeaway"}
